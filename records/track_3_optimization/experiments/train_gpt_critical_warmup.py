...
parser.add_argument("--aux-lr-mult", type=float, default=float(os.environ.get("AUX_LR_MULT", "1.0")))
parser.add_argument("--stop-step", type=int, default=None)
parser.add_argument("--data-dir", type=Path, default=Path(os.environ.get("DATA_DIR", "data/fineweb10B")))
...
parser.add_argument("--wandb-group", default=os.environ.get("WANDB_GROUP", "T3_CRITICAL_WARMUP_LR_PUSH_V1"))
...
FINAL_SCHEDULE_STEPS = args.final_schedule_steps   # t_end win (the +25-step SOTA lever; original public was 2960)
FINAL_LR_POWER = args.final_lr_power
...
EXPERIMENT_NAME = "t3-critical-warmup-lr-push"
EXPERIMENT_INTUITION = "Research harness for fixed Muon-only critical-warmup schedules on the #46 SOAP-Muon + Tail-EMA + RowFloor + CWD Track 3 stack."
MU = 0.95
BASE_MUON_LR = 0.0375
MUON_LR = BASE_MUON_LR * args.muon_lr_mult
TARGET_UW = 0.3825
...
TAILEMA_END = 2900
TAILEMA_LAMBDA = 0.6
STOP_STEP = args.stop_step if args.stop_step is not None else int(os.environ.get("STOP_STEP", FINAL_TRAIN_STEPS))   # default: full schedule; set env to early-stop
...
def distributed_data_generator(filename_pattern: str, batch_size: int, seq_len=1024):
    ...
    if not files:
        raise FileNotFoundError(f"No data shards matched {filename_pattern}")
...
print0(f"CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES', '')}")
...
print0(f"Using data_dir={args.data_dir}")
print0(f"Using schedule_kind={args.schedule_kind}")
print0(f"Using warmup_target={args.warmup_target}")
print0(f"Using warmup_steps={args.warmup_steps}")
print0(f"Using muon_lr_mult={args.muon_lr_mult}")
print0(f"Using aux_lr_mult={args.aux_lr_mult}")
...
wandb_run = None
if dist.get_rank() == 0 and args.wandb_mode != "disabled":
    try:
        import wandb
    except ImportError as exc:
        raise RuntimeError("W&B logging was requested but the wandb package is not installed.") from exc
    wandb_run = wandb.init(
        project=args.wandb_project,
        entity=args.wandb_entity,
        group=args.wandb_group,
        name=args.wandb_name or f"{args.schedule_kind}_w{args.warmup_steps}_m{args.muon_lr_mult}_seed{SEED}",
        mode=args.wandb_mode,
        config={
            "study_id": "T3_CRITICAL_WARMUP_LR_PUSH_V1",
            "base_record": "20260619_cwd_rowfloor_tailema",
            "seed": SEED,
...
def wandb_log(metrics, step: int):
...
train_loader = distributed_data_generator(str(args.data_dir / "fineweb_train_*.bin"), batch_size)
val_inputs, val_targets = next(distributed_data_generator(str(args.data_dir / "fineweb_val_*.bin"), val_tokens))
...
optimizer1 = AdamW([dict(params=[model.embed.weight], lr=0.3 * args.aux_lr_mult),
                    dict(params=[model.proj.weight], lr=(1/320) * args.aux_lr_mult)],
...
optimizer3 = Adam([
        dict(params=gain_aux_params, lr=0.01 * args.aux_lr_mult, betas=(0.8, 0.99)),
        dict(params=other_aux_params, lr=0.01 * args.aux_lr_mult, betas=(0.8, 0.997)),
        dict(params=attn_proj_bias_params, lr=0.01 * args.aux_lr_mult, betas=(0.8, 0.9965)),
    ],
...
def _interp_piecewise(x: float, points):
    if x <= points[0][0]:
        return points[0][1]
...
def warmup_multiplier(step: int) -> float:
    ...
    if args.schedule_kind == "linear":
        return progress
    if args.schedule_kind == "critical_table":
    ...
    if args.schedule_kind == "catapult_proxy":
        return _interp_piecewise(
            progress,
            [
                (0.00, 0.10),
    ...
    raise ValueError(f"Unsupported schedule_kind={args.schedule_kind}")
...
for step in range(train_steps + 1):
    ...
    if should_validate:
        ...
        val_ema_loss_float = float(val_ema_loss.item())
        if first_val_ema_below_3p28 is None and val_ema_loss_float <= 3.28:
            first_val_ema_below_3p28 = step
        summary = {
            "event": "validation",
            "step": step,
            "train_steps": train_steps,
            "val_loss": val_loss_float,
            "val_ema_loss": val_ema_loss_float,
            "first_val_ema_below_3p28": first_val_ema_below_3p28,
        ...
        wandb_log({
            "val_loss": val_loss_float,
            "val_ema_loss": val_ema_loss_float,
            "first_val_ema_below_3p28": first_val_ema_below_3p28 or -1,
            "train_time": training_time,
            "step_avg_ms": 1000 * training_time / max(step, 1),
            "warmup_mult": warmup_multiplier(step),
            "muon_lr": optimizer2.param_groups[0]["lr"],
    ...
    for i in range(len(inputs) // mbs):
        ...
        if not torch.isfinite(loss).all():
            ...
            print0("SUMMARY_JSON " + json.dumps(summary, sort_keys=True), console=True)
            wandb_log({"stable": 0, "train_nonfinite_step": step}, step)
